...
in_queue(s::JobState) =
    s in (Submitted, Pending, Running, Configuring, Completing, Suspended)
...
function maybe_restart_allocs(sc::HQ)
    ...
    if length(alloc_lines) == 3 # no allocs -> add all
        ...
        alloc_args = map(a -> replace(strip(split(a, "|")[end-1]), "," => " "), alloc_lines[4:end-1])
