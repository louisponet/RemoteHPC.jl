...
function Base.write(io::IO, job_info::Tuple, sched::Scheduler)
    ...
    write(io, "# Environment: name=$(e.name) type=$(typeof(e))\n")
    ...
    for c in calcs
        ...
        write(io, c)
        write(io," #exec: names=[$names] ")
        if !isempty(c.exec.modules)
            write(io, "modules=[")
            for m in c.exec.modules[1:end-1]
                write(io, "$m ")
            end
            write(io, "$(c.exec.modules[end])]\n")
        end
    end
    
    write_postamble_string(io, e)
end
...
function parse_script(str::AbstractString, sched::Scheduler)
    ...
    dirs, preamble = directives(preamble, sched)
    job_name = pop!(dirs,name_directive(sched), "noname") 
    return (name=job_name, environment=Environment(name=name, exports=exports, preamble=preamble, directives=dirs, parallel_exec=parallel_exec), calculations=calcs)
end

function directives(preamble::String, sched::Scheduler)
    ...
    return params, preamble[last+1:end]
end
